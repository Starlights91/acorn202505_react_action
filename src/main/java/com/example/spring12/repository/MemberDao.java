package com.example.spring12.repository; //#m 클래스가 속한 패키지 경로. repository 패키지 → 보통 DB 접근 계층(DAO) 을 담는 위치

import java.util.List; //#m 회원 목록을 여러 명 담아 반환할 때 필요 (List 컬렉션 타입을 사용하기 위해 import.)

import com.example.spring12.dto.MemberDto; //#m DB의 row 데이터를 자바 객체로 옮겨 담는 역할
